...
var init = () => {
...

    {
        ...
        q1Exp.description = Localization.getUpgradeIncCustomExpDesc("q_1", "1");
        q1Exp.info = Localization.getUpgradeIncCustomExpInfo("q_1", "1");
...
var getPrimaryEquation = () => {
    ...
    result += "\\dot{\\rho}=q_1";
    if (q1Exp.level > 0) result += `^{${1+q1Exp.level*1}}`;
...
var getPublicationMultiplier = (tau) => tau.isZero ? BigNumber.ONE : tau.pow(BigNumber.from(3));
var getPublicationMultiplierFormula = (symbol) => "{" + symbol + "}^{13}";
...
var getQ1Exp = (level) => BigNumber.from(1 + level * 1);
